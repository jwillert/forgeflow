...
Read this project's domain and architecture docs if present (`CONTEXT.md`, `CLAUDE.md`, `docs/adr/`, or similar) before resolving anything substantive.
...
After resolving, run this project's standard build/test command (check `package.json` scripts, a `Makefile`, `CONTRIBUTING.md`, or `CLAUDE.md` for what that is) — the repo-wide check is strongest; a narrower/focused check is fine if the resolution is localized. If something is broken, fix what you can; if you can't, push what you have and flag it clearly in the comment.
